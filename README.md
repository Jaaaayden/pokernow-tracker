...
```
549 hands · 9,206 log entries · 0 parse misses · 0 pot mismatches · 84 tests passing
```
...
pnt import ~/Downloads/poker_now_log_*.csv   # safe to re-run; duplicates are ignored
...
1. **`bets N`, `raises to N` and `calls N` are all cumulative for the street**, not
   incremental. Get this wrong and every pot, net-won figure and bb/100 is wrong
   while still looking plausible. Guarded by a conservation law over all 549 hands.
